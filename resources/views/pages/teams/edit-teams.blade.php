    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-grow-1 fs-3 fw-semibold my-2 my-sm-3">Edit Teams </h1>
    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            ...
            <div class="block-content">
                <div class="row push">
                    <div class="col-lg-8">
                        <form action="{{route('teams.update',$editTeams->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                <label class="form-label" for="example-text-input">Team Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="val-name" name="name" value="{{$editTeams->name}}" placeholder=" Team Name">
                                @error('name')
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="example-select" name="league_id">
                                    @foreach($league as $newLeague)
                                        @if ($newLeague->id  == $editTeams->league_id)
                                            <option  checked="" value="{{$newLeague->id}}">{{$newLeague->name}}</option>
                                        @else
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control" id="val-name" name="location" value="{{$editTeams->location}}" placeholder=" Team location">
                                @error('location')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control" id="val-name" name="coach_name" value="{{$editTeams->coach}}" placeholder=" Team Coach Name">
                                @error('coach_name')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
